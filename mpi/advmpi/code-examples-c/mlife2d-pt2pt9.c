...
int MLIFE_ExchangePt2pt9( MLIFEPatchDesc *patch, int **matrix, MLIFETiming *timedata, void *privateData )
{
    ...
    MPI_Isend(&matrix[1][1],       1, type, patch->left,  0, comm, reqs  );
    MPI_Irecv(&matrix[1][0],       1, type, patch->left,  0, comm, reqs+1);
    MPI_Isend(&matrix[1][LCols],   1, type, patch->right, 0, comm, reqs+2);
    MPI_Irecv(&matrix[1][LCols+1], 1, type, patch->right, 0, comm, reqs+3);

    /* move the top, bottom edges */
    MPI_Isend(&matrix[1][1],       LCols, MPI_INT, patch->up,   0, comm, reqs+4);
    MPI_Irecv(&matrix[0][1],       LCols, MPI_INT, patch->up,   0, comm, reqs+5);
    MPI_Isend(&matrix[LRows][1],   LCols, MPI_INT, patch->down, 0, comm, reqs+6);
    MPI_Irecv(&matrix[LRows+1][1], LCols, MPI_INT, patch->down, 0, comm, reqs+7);
    ...
    MPI_Irecv(&matrix[LRows+1][LCols+1], 1, MPI_INT, patch->lr, 0, comm, reqs+15);
