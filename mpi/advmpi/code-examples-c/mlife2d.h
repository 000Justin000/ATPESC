...
typedef struct 
{
    ...
    int left, right, up, down;      /* Neighbors to this patch: left = (I,J-1), up = (I-1,J), etc.  (standard matrix ordering) */
    int ul, ur, ll, lr;             /* Upper left, upper right, lower left, lower right (needed for one option of the 9 point stencil) */
    int gNI, gNJ;                   /* Full size of data mesh */
    int gI, gJ;                     /* Global I,J for the upper left corner of the local patch */
    int lni, lnj;                   /* Size of local patch */
} MLIFEPatchDesc;

typedef struct 
{
    double packtime;                /* Time to pack data if separate */
    double unpacktime;              /* Time to unpack data if separate */
...
int MLIFE_PatchCreateProcessMeshWithCart( MLIFEOptions *options, MLIFEPatchDesc *patch );
int MLIFE_PatchCreateDataMeshDesc( MLIFEOptions *options, MLIFEPatchDesc *patch );
...
int MLIFE_TimeIterations( MLIFEPatchDesc *patch, int nIter, int doCheck, int **m1, int **m2, 
...
// ------------------------------------------------------------------------------------------------------------------------------------
// different version of communication
// ------------------------------------------------------------------------------------------------------------------------------------
...
int MLIFE_ExchangeInitPt2ptSnd( MLIFEPatchDesc *patch, int **m1, int **m2, void *privateData );
int MLIFE_ExchangePt2ptSnd( MLIFEPatchDesc *patch, int **matrix, MLIFETiming *timedata, void *privateData );
...
int MLIFE_ExchangeInitPt2pt9( MLIFEPatchDesc *patch, int **m1, int **m2, void *privateData );
int MLIFE_ExchangePt2pt9( MLIFEPatchDesc *patch, int **matrix, MLIFETiming *timedata, void *privateData );
